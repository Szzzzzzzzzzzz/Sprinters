	...
	public function start_el( &$output, $taxonomy_term, $depth = 0, $args = array(), $current_object_id = 0 ) {
        ...
        if ( $this->in_this_tax_archive_page ) {
            ...
            if ( $this->tax_archive_term->slug == $taxonomy_term->slug ) {
                ...
                $output .= '<option value="' . esc_attr( $tax_value ) . '" selected="selected">' . $indent . esc_html( $taxonomy_term->tax_option ) . "</option>\n";
            }
			...
			$option_args = array(
				'label'			=> $indent . esc_html( $taxonomy_term->tax_option ),
				'attributes'	=> array(
					'value'		=> $tax_value
				)
			);
